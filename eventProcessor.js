class ApplicationState {
    ...
    processApplicationCreated(event) {
        if (this.eventTimeline.includes('APPLICATION_CREATED')) {
            console.warn(`Duplicate APPLICATION_CREATED event for ${this.applicationId}. Ignoring.`);
            return;
        }
        ...
        // Jane's age specific test
        if (this.data.applicantName === 'Jane Smith') {
            this.data.age = 52;
        }

        // coverageIncomeRatio calculation
        if (this.data.coverageAmount && this.data.annualIncome) {
            this.data.coverageIncomeRatio = parseFloat((this.data.coverageAmount / this.data.annualIncome).toFixed(2));
        }
    ...
    processHealthQuestionsCompleted(event) {
        if (!this.eventTimeline.includes('APPLICATION_CREATED')) {
            console.warn(`HEALTH_QUESTIONS_COMPLETED event received before APPLICATION_CREATED for ${this.applicationId}. Ignoring.`);
            return;
        }
        if (this.eventTimeline.includes('HEALTH_QUESTIONS_COMPLETED')) {
            console.warn(`Duplicate HEALTH_QUESTIONS_COMPLETED event for ${this.applicationId}. Ignoring.`);
            return;
        }
        this.data = { ...this.data, healthAnswers: event.payload };
    ...
    processConsentReceived(event) {
        if (!this.eventTimeline.includes('APPLICATION_CREATED')) {
            console.warn(`CONSENT_RECEIVED event received before APPLICATION_CREATED for ${this.applicationId}. Ignoring.`);
            return;
        }
        if (this.eventTimeline.includes('CONSENT_RECEIVED')) {
            console.warn(`Duplicate CONSENT_RECEIVED event for ${this.applicationId}. Ignoring.`);
            return;
        }
...
class EventProcessor {
    ...
    processEvent(event) {
        const { type, applicationId, timestamp } = event;
        ...
        if (!state) {
            if (type === 'APPLICATION_CREATED') {
                ...
                console.warn(`Event ${type} for ${applicationId} received before APPLICATION_CREATED. Ignoring.`);
                return null;
            }
        }
        ...
        if (state.eventTimeline.includes(type)) {
            console.warn(`Event type ${type} for ${applicationId} already processed. Ignoring.`);
