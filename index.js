      ...
      try {
        ...
        if (caseProcessedEvent) {
          await producer.send({
            ...
            messages: [
              {
                value: JSON.stringify(caseProcessedEvent),
                key: caseProcessedEvent.applicationId,
              },
            ],
          });
          console.log(
            `Produced CASE_PROCESSED event for application ${caseProcessedEvent.applicationId} with status ${caseProcessedEvent.validation.canProceedToReadiness ? "can proceed" : "cannot proceed"}`,
          );
        }
